  ...
  app.get('/api/health', async (req, res, next) => {
    try {
      ...
      try {
        ...
        console.error('[HEALTH] Firebase Check failed:', fbErr.message);
      }

      const memoryUsage = process.memoryUsage();
      const uptime = process.uptime();

      const isHealthy = dbStatus === 'connected' && firebaseStatus === 'connected';
  ...
  app.get('/api/auth/me', authLimiter, requireAuth, async (req: AuthRequest, res, next) => {
    try {
      await createLogRecord(
        ...
        'Authentication',
        `User ${req.user?.username} verified via Firebase JWT.`
      );
