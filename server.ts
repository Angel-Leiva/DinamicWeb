...
import { users, refreshTokens } from './src/db/schema.ts';
...
async function startServer() {
  ...
  app.get('/api/versions', requireAuth, async (req, res, next) => {
    try {
      const list = await getAllVersionsList();
      res.json(list);
    } catch (error) {
      next(error);
    }
  ...
  app.delete('/api/submissions/:id', requireAuth, requirePermission('forms:delete'), async (req: AuthRequest, res, next) => {
    try {
      const { id } = req.params;
      const deleted = await deleteFormSubmissionRecord(id);
      if (!deleted) {
        throw new AppError('Envío de formulario no encontrado.', 404);
      }
      await createLogRecord(
        req.user?.email || 'System',
        'Form Deletion',
        `Deleted form submission ID ${id} for form '${deleted.formTitle}'`
      );
      res.json(deleted);
    } catch (error) {
      next(error);
  ...
  app.post('/api/users', usersLimiter, requireAuth, requirePermission('admins:manage'), async (req: AuthRequest, res, next) => {
    try {
      ...
      const { username, email, roleId, password, confirmPassword } = parseResult.data;

      if (password && confirmPassword && password !== confirmPassword) {
        throw new AppError('La contraseña y su confirmación no coinciden.', 400);
      }

      const newUser = await createStaffUser(username, email, roleId, password);
      await createLogRecord(
        req.user?.email || 'System',
        'Staff Creation',
  ...
  app.post('/api/users/:uid/reset-password', usersLimiter, requireAuth, requirePermission('admins:manage'), async (req: AuthRequest, res, next) => {
    try {
      ...
      if (!password || !confirmPassword) {
        throw new AppError('La contraseña y su confirmación son requeridas.', 400);
      }
      if (password !== confirmPassword) {
      ...
      await createLogRecord(
        ...
        `Restableció la contraseña del usuario UID ${uid}`
      );
