    ...
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      if (firebaseUser) {
        try {
          ...
          // Fetch real user record from backend
          ...
          if (res.ok) {
            ...
            await loadDatabaseContent(token);
          } else {
            console.error('Failed to verify backend user session');
            setCurrentUser(null);
            setAuthToken(null);
          }
        } catch (error) {
          console.error('Error during automatic auth recovery:', error);
          setCurrentUser(null);
          setAuthToken(null);
        }
      } else {
