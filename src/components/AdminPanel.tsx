...
export default function AdminPanel({
  ...
  // RBAC Config States (Loaded exclusively from PostgreSQL)
  const [rolesList, setRolesList] = useState<Role[]>([]);
  const [usersList, setUsersList] = useState<User[]>([]);
  const [permissionsList, setPermissionsList] = useState<{ id: string; name: string; description: string }[]>([]);

  // Fetch users, roles, and permissions from PostgreSQL database
  useEffect(() => {
    if (!authToken) return;
    ...
    const fetchPermissions = async () => {
      try {
        ...
        if (res.ok) {
          const dbPermissions = await res.json();
          setPermissionsList(dbPermissions);
        }
      } catch (e) {
  ...
  const handleSavePageBlocks = async (blockData: Block[], summary: string, publish: boolean) => {
    if (!editingPageId) return;

    const pg = pages.find(p => p.id === editingPageId);
    if (!pg) return;

    if (publish && !hasPermission('pages:publish')) {
      triggerToast('Error: No tienes permisos para publicar versiones.', 'error');
      return;
    } else if (!publish && !hasPermission('pages:edit')) {
      triggerToast('Error: No tienes permisos para guardar borradores.', 'error');
      return;
    }
    ...
    try {
      ...
      // 1. PUT update to page
      const res = await fetch(`/api/pages/${pg.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || err.message || 'Error actualizando página.');
      }

      const updatedPageFromServer = await res.json();

      // 2. If publishing, POST the version snapshot
      if (publish) {
        ...
        if (!verRes.ok) {
          ...
          throw new Error(err.error || err.message || 'Error guardando versión.');
        }

        const savedVersionFromServer = await verRes.json();
        setVersions((prev) => [savedVersionFromServer, ...prev]);
        onAddLog('Publicación de Versión', `Se publicó la versión v${nextVer} de la página "${pg.title}".`);
        triggerToast(`¡Éxito! Se publicó la versión v${nextVer} de la página.`, 'success');
      } else {
        onAddLog('Guardado de Borrador', `Se guardó un borrador de los bloques para "${pg.title}".`);
        triggerToast('Borrador guardado exitosamente.', 'success');
      }

      // Update pages state from server response
      setPages((prevPages) =>
        prevPages.map((p) => p.id === pg.id ? updatedPageFromServer : p)
      );
  ...
  const handleRollback = async (ver: PageVersion) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error realizando rollback.');
      }
  ...
  const handleTogglePublish = async (pageId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al cambiar estado de publicación.');
      }

      const updatedPageFromServer = await res.json();

      setPages((prevPages) =>
        prevPages.map((p) => p.id === pageId ? updatedPageFromServer : p)
      );

      onAddLog('Cambio de Publicación', `Página "${pg.title}" cambiada a ${nextState ? 'Publicada' : 'Borrador'}.`);
      triggerToast(`Página marcada como ${nextState ? 'Publicada' : 'Borrador'} exitosamente.`, 'success');
    } catch (err: any) {
      triggerToast(err.message, 'error');
    }
  };
  ...
  const handleCreatePageSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al crear página.');
      }

      const createdPageFromServer = await res.json();

      setPages((prev) => [...prev, createdPageFromServer]);
      onAddLog('Creación de Página', `Se creó la página "${newPageTitle}" como Borrador.`);
      triggerToast('Página creada exitosamente.', 'success');
  ...
  const handleDeletePage = async (pageId: string) => {
    ...
    if (confirm('¿Estás seguro de que deseas eliminar permanentemente esta página? Esta acción es irreversible.')) {
      ...
      try {
        ...
        if (!res.ok) {
          const err = await res.json();
          throw new Error(err.error || err.message || 'Error al eliminar página.');
        }

        setPages((prev) => prev.filter((p) => p.id !== pageId));
        onAddLog('Eliminación de Página', `Página "${pageToDelete?.title}" eliminada permanentemente.`);
        triggerToast('Página eliminada exitosamente.', 'success');
      } catch (err: any) {
        triggerToast(err.message, 'error');
      }
    }
  ...
  const handleTogglePermission = async (roleId: string, permId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al actualizar permisos de rol.');
      }

      setRolesList((prevRoles) =>
        prevRoles.map((r) => r.id === roleId ? { ...r, permissions: nextPerms } : r)
      );

      onAddLog('Modificación de Roles (RBAC)', `Se alteró el permiso "${permId}" del rol "${role.name}".`);
      triggerToast(`Permisos del rol '${role.name}' actualizados exitosamente.`, 'success');
    } catch (err: any) {
      triggerToast(err.message, 'error');
    }
  };
  ...
  const handleMediaUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al subir recurso.');
      }

      const savedMediaFromServer = await res.json();

      setMedia((prev) => [savedMediaFromServer, ...prev]);
      onAddLog('Carga de Archivo', `Se subió la imagen "${mediaName}" al almacén S3.`);
      triggerToast('Imagen guardada exitosamente.', 'success');
  ...
  const handleDeleteMedia = async (mediaId: string) => {
    if (!hasPermission('media:delete')) {
      triggerToast('Error: No tienes permisos para borrar archivos.', 'error');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al borrar recurso.');
      }

      setMedia((prev) => prev.filter((m) => m.id !== mediaId));
      onAddLog('Eliminación de Archivo', `Se eliminó el archivo "${file.name}" del almacén S3.`);
      triggerToast('Archivo multimedia eliminado permanentemente.', 'success');
    } catch (err: any) {
      triggerToast(err.message, 'error');
    }
  };
  ...
  const handleDeleteSubmission = async (subId: string) => {
    if (!hasPermission('forms:delete')) {
      triggerToast('Error: No tienes permisos para eliminar envíos de formularios.', 'error');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || err.message || 'Error al eliminar el envío.');
      }

      setSubmissions((prev) => prev.filter((s) => s.id !== subId));
      triggerToast('Envío de formulario eliminado permanentemente.', 'success');
      onAddLog('Eliminación de Envío', `Se eliminó el envío de formulario ID ${subId}.`);
    } catch (err: any) {
      triggerToast(err.message, 'error');
    }
  };
