...
export default function AdminPanel({
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onAddLog('Inicio de Sesión', `El usuario ${data.user.username} (${data.user.email}) accedió vía Google.`);
      } else {
        const errData = await res.json();
        setLoginError(errData.error || 'No se pudo sincronizar el usuario con la base de datos.');
      }
    } catch (error: any) {
      console.error('Error con Google Sign-In:', error);
      setLoginError(error.message || 'Error al autenticar con Google. Por favor, reintente.');
    }
  };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    // Check pre-defined mock admin credentials
    if (usernameInput === 'AngelLeiva' && passwordInput === 'Al22-2005') {
      const u = usersList.find(user => user.username === 'AngelLeiva') || INITIAL_USERS[0];
      onLoginSuccess(u);
      onAddLog('Inicio de Sesión', 'El Super Administrador General (AngelLeiva) accedió al panel.');
    } else {
  ...
  const handleCreateUserSubmit = (e: React.FormEvent) => {
    ...
    const usrRand = Math.random().toString(36).substring(2, 6);
    const newUser: User = {
      id: `usr-${Date.now()}-${usrRand}`,
  ...
  const handleDeleteUser = (userId: string) => {
    ...
    onAddLog('Eliminación de Usuario', `Usuario administrativo "${userToDelete?.username}" removido.`);
  };
        ...
        <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 sm:p-8 shadow-md">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div className="relative flex py-2 items-center">
              ...
              <span className="flex-shrink mx-4 text-[10px] text-gray-400 font-mono uppercase">O también</span>
              <div className="flex-grow border-t border-gray-200 dark:border-gray-800"></div>
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer"
              id="google-login-btn"
            >
        ...
        <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-sm min-h-[400px]">
          ...
          {activeTab === 'users' && (
            <div className="space-y-6 animate-fade-in">
              ...
              {isCreatingUser && (
                <form onSubmit={handleCreateUserSubmit} className="p-4 border border-blue-100 dark:border-blue-900 bg-blue-50/20 dark:bg-blue-950/20 rounded-2xl space-y-4">
              ...
              <div className="border border-gray-100 dark:border-gray-900 rounded-2xl overflow-hidden">
                ...
                <div className="divide-y divide-gray-100 dark:divide-gray-900 text-xs">
                  {usersList.map((usr) => (
                    <div key={usr.id} className="grid grid-cols-4 p-3 items-center hover:bg-gray-50/50">
                      <div>
                      ...
                      <div className="text-right">
                        {hasPermission('admins:manage') && usr.id !== currentUser.id && (
                          <button
