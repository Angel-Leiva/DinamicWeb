  ...
  const renderBlock = (block: Block) => {
    switch (block.type) {
      ...
      case 'image':
        ...
        return (
          <div className="my-5 flex flex-col items-center">
            <div className={`relative overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-900 w-full max-w-3xl ${getAspectClass(aspectRatio)}`}>
              <img 
                src={url || 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=800'} 
                alt={caption || 'CMS Upload'} 
                className={`w-full h-full ${aspectRatio && aspectRatio !== 'auto' && aspectRatio !== 'original' && aspectRatio !== 'free' ? 'object-cover' : 'object-contain'}`}
      ...
      case 'video':
        ...
        const vUrl = block.video.url;
        const vProvider = block.video.provider || '';
        const resolvedProvider = getPlayerType(vUrl, vProvider);
        const ytEmbed = getYouTubeEmbedUrl(vUrl);
        const vmEmbed = getVimeoEmbedUrl(vUrl);

        return (
          <div className="my-5 max-w-3xl mx-auto w-full">
            <div className="relative aspect-video rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-800 bg-black flex items-center justify-center shadow-md">
              {resolvedProvider === 'youtube' && ytEmbed ? (
                <iframe
                  src={ytEmbed}
                  ...
                  allowFullScreen
                />
              ) : resolvedProvider === 'vimeo' && vmEmbed ? (
                <iframe
                  src={vmEmbed}
                ...
                {block.form.fields.map((fld) => {
                  ...
                  return (
                    <div key={fld.id} className="space-y-1">
                      {fld.type !== 'radio' && fld.type !== 'multiselect' && (
                      ...
                      {fld.type === 'textarea' ? (
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex flex-col gap-1.5 pt-0.5">
                            {(fld.options || []).map((opt) => (
                              <label key={opt} className="flex items-center gap-2 cursor-pointer text-xs text-gray-650 dark:text-gray-450">
                                <input
                                  type="radio"
                                  name={inputId}
                                  value={opt}
                                  checked={currentVal === opt}
                                  required={fld.required && !currentVal}
                                  onChange={(e) => handleInputChange(block.id, fld.id, e.target.value)}
                                ...
                                <label key={opt} className="flex items-center gap-2 cursor-pointer text-xs text-gray-650 dark:text-gray-450">
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      handleInputChange(block.id, fld.id, nextList.join(', '));
                                    }}
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          ...
                          <label htmlFor={inputId} className="text-xs text-gray-600 dark:text-gray-400">
                            {fld.label || 'Acepto términos y condiciones'}
                          </label>
                        </div>
                      ) : (
                        <input
